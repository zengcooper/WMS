...
namespace Git.Storage.Provider.Base
{
    public partial class PowerProvider : DataFactory
    {
        ...
        private List<SysRelationEntity> GetList()
        {
            ...
            if (!list.IsNullOrEmpty())
            {
                //netcore不支持sysweb，cache只用2个参数的函数,后期注意权限变更到期时间
                CacheHelper.Insert(CacheKey.JOOSHOW_ALLOTPOWER_CACHE, list);
            }
            return list;
        }
        ...
        public List<SysResourceEntity> GetRoleResource(string RoleNum)
        {
            ...
            if (ListResult.IsNullOrEmpty())
            {
                ...
                {
                    //netcore不支持sysweb，cache只用2个参数的函数,后期注意权限变更到期时间
                    CacheHelper.Insert(string.Format(CacheKey.JOOSHOW_ROLEPOWER_CACHE, RoleNum), ListResult);
                }
            }
            return ListResult;
        }
